...
from devopspilot.contracts.providers import CICapability, ReviewState, SCMCapability
...
class FakeCNBClient:
    async def request_json(self, method: str, path: str, *, body=None, query=None) -> Any:
        ...
        if method=="POST" and path=="/acme/demo/-/issues/9/comments":
            return {"id":"c1"}
        if method=="POST" and path=="/acme/demo/-/pulls/9/reviews":
            assert body["event"]=="approve"; return None
...
async def main():
    ...
    pr=await scm.create_change_request(repo,title="Fix CI",body="",source_branch="fix/ci",target_branch="main")
    assert pr.source_branch=="fix/ci"
    review=await scm.submit_review(repo,change_id="9",state=ReviewState.APPROVE,body="Verified")
    ...
    print("CNB_CAPABILITY_DISCOVERY_OK")
